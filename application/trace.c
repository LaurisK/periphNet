...
static void trace_tcp_write(const uint8_t *enc, size_t encLen);
...
void trace_init(void)
{
    ...
    UserNonBlockingDeferredWrite8AuxiliaryFn = trace_tcp_write;
...
static void trace_task(void *argument)
{
    ...
    while (1) {
        ...
        counter++;
        TRice(iD(6862), "Counter is - %d\n", counter);

        if (counter % 20 == 0) {
            TaskHandle_t tcpip_h = xTaskGetHandle("tcpip_thread");
            if (tcpip_h) {
                TRice(iD(3891), "tcpip HWM=%u words\n",
                      (unsigned)uxTaskGetStackHighWaterMark(tcpip_h));
            }
            TRice(iD(4404), "upload HWM=%u words heap=%u\n",
...
static err_t trace_accept_callback(void *arg, struct tcp_pcb *newpcb, err_t err)
{
    ...
    TRice(iD(1019), "New client received for trice stream.");
 ...
 * @note Assigned to UserNonBlockingDeferredWrite8AuxiliaryFn in trace_init().
 */
static void trace_tcp_write(const uint8_t* enc, size_t encLen)
{
    if (encLen == 0) {
