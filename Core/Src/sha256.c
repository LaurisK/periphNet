 ...
 * Optimised for ARM Cortex-M4 (32-bit operations, no 64-bit deps).
 */
...
static void sha256_transform(sSha256Ctx *ctx, const uint8_t block[64])
{
    uint32_t W[64];
...
{
    uint32_t buf_used = (uint32_t)(ctx->count & 0x3F);
    ctx->count += len;

    /* Fill partial block */
    if (buf_used > 0) {
        uint32_t space = SHA256_BLOCK_SIZE - buf_used;
        if (len < space) {
...
void sha256_final(sSha256Ctx *ctx, uint8_t digest[SHA256_DIGEST_SIZE])
{
    ...
    /* If not enough room for length, pad + transform + new block */
    if (buf_used > 56) {
    ...
    /* Pad with zeros up to bit-length field */
